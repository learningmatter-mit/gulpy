...
class TestParser(ut.TestCase):
    def setUp(self):
        self.parser = PropertyParser.from_file(get_jobs_path("opti/opti.out"))
...
class TestMoleculeParser(ut.TestCase):
    def setUp(self):
        self.parser = PropertyParser.from_file(get_jobs_path("opti/molecule.out"))

    def test_energy(self):
        energy = self.parser.get_total_energy()
        self.assertAlmostEqual(energy, 2.91123336)

    def test_gnorm(self):
        gnorm = self.parser.get_gnorm()
        self.assertAlmostEqual(gnorm, 0.00005011)

    def test_forces(self):
        forces = self.parser.get_forces()
        expected = np.array(
            [
                [-0.000292, -0.000816, -0.000127],
                [-0.000291, 0.000225, -0.000349],
                [0.000673, 0.000628, 0.000538],
                [0.00126, 0.000221, 0.001868],
                [0.0, 0.0, 0.0],
                [-0.000169, -9.9e-05, -0.000341],
                [-1.6e-05, 3.8e-05, -8.3e-05],
                [8.3e-05, 9e-06, -0.000147],
                [0.00013, 0.001458, -9.6e-05],
                [-7.8e-05, 2e-06, -9.6e-05],
                [-0.001363, -6.8e-05, 0.000149],
                [-0.000151, -1.3e-05, -0.00017],
                [5.8e-05, -0.000332, 1e-05],
                [-3.9e-05, 0.000198, -0.000434],
                [-0.000182, 0.000627, -0.000116],
                [-0.001143, -0.000644, -0.000636],
                [0.000693, 9.6e-05, 0.001044],
                [-0.000315, 0.000586, 0.001007],
                [0.001345, -0.000708, -0.000678],
                [-0.000798, -0.00118, -9.5e-05],
                [0.000252, 0.000439, -0.001153],
                [-0.000874, 0.001046, -0.000392],
                [-0.000473, -0.000191, 0.001084],
                [0.000186, 1.1e-05, -0.000201],
                [-0.000106, -0.000101, -0.000215],
                [0.000368, -0.000196, 3.5e-05],
                [8.5e-05, -4e-06, -3.7e-05],
                [9.1e-05, -0.000242, 0.000201],
                [-0.000169, -0.001045, 0.000271],
                [-0.000343, 8.6e-05, -7.4e-05],
                [0.000157, 4e-06, 5.8e-05],
                [0.00093, 0.000185, 0.000311],
                [0.000246, -7.8e-05, 0.000329],
                [8e-06, -0.000136, -1e-06],
                [0.000223, -0.000103, -0.000414],
                [-5e-06, 0.000208, -0.000106],
                [0.000202, -7.8e-05, -0.00031],
                [-0.000136, -2.8e-05, -0.000384],
            ]
        )
        self.assertIsNone(
            np.testing.assert_almost_equal(np.array(forces), expected, decimal=5)
