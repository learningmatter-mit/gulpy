...
class CatlowLabels(Labels):
    ...
    def has_shell(self, structure: Structure):
        return [label == "O_O2-" for label in self.get_labels(structure)]

    def get_labels(self, structure: Structure):
        return [self.get_site_label(structure, site) for site in structure.sites]

    def get_site_label(self, structure: Structure, site: Site):
        assert (
            site.species_string in self.ELEMENTS
        ), "element {} not in SLC library".format(site.species_string)
