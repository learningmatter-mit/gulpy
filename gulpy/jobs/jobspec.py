...
class DockingJobspec(GulpJobspec):
    ...
    def get_molecule_labels(self):
        ...
        for indices in self.molecule_indices:
            ...
            dreiding_labeler = DreidingTypes(
                self.smiles,
                xyz
            )
    ...
    def get_all_labels(self):
        labels = {
            **self.get_molecule_labels(),
            **self.get_substrate_labels()
        }
        return {
            k: labels[k]
    ...
    def get_molecule_bonds(self, is_complex=True):
        ...
        for indices in self.molecule_indices:
            ...
            if is_complex:
                bonds += [
                    (indices[u] + 1, indices[v] + 1, btype)
                    for u, v, btype in mol_bonds
                ]
            else:
                bonds += [
                    (u + 1 + cumsum, v + 1 + cumsum, btype)
                    for u, v, btype in mol_bonds
                ]
                cumsum += len(indices)
    ...
    def get_renamed_structure(self, labels):
        species = [
            self.structure.species[idx]
            for idx in labels.keys()
        ]
        coords = [
            self.structure.cart_coords[idx]
            for idx in labels.keys()
        ]
        return Structure(
            species=species,
